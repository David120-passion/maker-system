 ...
 */
public final class InternalRangeOscillatorStrategy implements MarketMakingStrategy {
    ...
    private final double momentumFactor;     // 动量因子（保留以保持向后兼容，新算法已不使用）
    ...
    private static final double MIN_CORRECTION_INTERVAL_RATIO = 0.015;            // 最小回调间隔：周期时长的 1.5%
    private static final double MAX_CORRECTION_INTERVAL_RATIO = 0.08;             // 最大回调间隔：周期时长的 8%
    private static final double MIN_CORRECTION_AMPLITUDE_PERCENT = 2.0;           // 最小回调幅度：2%
    private static final double MAX_CORRECTION_AMPLITUDE_PERCENT = 15.0;           // 最大回调幅度：8%
    private static final double MIN_CORRECTION_DURATION_RATIO = 0.017;            // 最小回调持续时间：周期时长的 0.8%
    private static final double MAX_CORRECTION_DURATION_RATIO = 0.04;             // 最大回调持续时间：周期时长的 4%
    private static final double CONVERGENCE_THRESHOLD_PERCENT = 5.0;              // 收敛阈值：5%（如果进度落后超过此值，加速）
    private static final double NOISE_FACTOR = 0.3;// 噪声因子：30% 原始波动率
     ...
     * @param momentumFactor 动量因子（保留以保持向后兼容）
     ...
     */
    public InternalRangeOscillatorStrategy(
        ...
        if (momentumFactor < 0.0 || momentumFactor > 1.0) {
            throw new IllegalArgumentException("momentumFactor 必须在 0.0~1.0 之间");
        }
        ...
        LOG.info("InternalRangeOscillatorStrategy 已创建: minPrice={}, maxPrice={}, tickSize={}, volatilityPercent={}% (实际波动≈{}), " +
    ...
    public void onAccountOrderUpdate(CoreEvent event) {
        ...
        if (event.orderStatus == 7) { // Rejected
            LOG.warn("策略订单被拒绝: orderId={}, symbolId={}, side={}, price={}, qty={}",
                event.localOrderId,
                event.symbolId,
        ...
        // 统计成交量（订单成交时）
        if (event.orderStatus == 2 || event.orderStatus == 3) { // Filled 或 PartiallyFilled
            if (event.filledQty > 0 && event.symbolId == symbolId) {
                accumulatedVolumeE8 += event.filledQty/2;
                if (enableVolumeTarget && accumulatedVolumeE8 >= targetVolumeE8) {
                    LOG.info("已达到目标成交量: {} BTC (目标: {} BTC)",
                        accumulatedVolumeE8 / (double) ScaleConstants.SCALE_E8,
                        targetVolumeE8 / (double) ScaleConstants.SCALE_E8);
                }
                LOG.info("当前总成交量{}",accumulatedVolumeE8);
            }
        }
    }
    ...
    public void onTimer(CoreEvent event) {
        ...
        if (!initialized) {
            ...
            initialized = true;
            LOG.info("策略已初始化，起始价格: {}（从最小值开始上涨），周期时长: {} 小时，目标量: {} BTC",
    ...
    private double calculateNewTargetPrice(long currentTimeMs) {
        ...
        double noiseE8 = random.nextGaussian() * baseVolatilityE8 * NOISE_FACTOR;
    ...
    private double calculateBaseTrendWithSigmoid(double progress, double priceRangeE8) {
        if (progress < 0.5) {
            // 前 50%：上涨周期
            double upProgress = progress * 2.0; // 0.0 ~ 1.0
            ...
            // 后 50%：下跌周期
            double downProgress = (progress - 0.5) * 2.0; // 0.0 ~ 1.0
    ...
    private void updateCorrectionState(long currentTimeMs, double progress, double baseTrendE8, double priceRangeE8) {
        ...
        // 根据周期时长动态计算回调间隔和持续时间
        long minCorrectionIntervalMs = (long) (cycleDurationMs * MIN_CORRECTION_INTERVAL_RATIO);
        long maxCorrectionIntervalMs = (long) (cycleDurationMs * MAX_CORRECTION_INTERVAL_RATIO);
        long minCorrectionDurationMs = (long) (cycleDurationMs * MIN_CORRECTION_DURATION_RATIO);
        long maxCorrectionDurationMs = (long) (cycleDurationMs * MAX_CORRECTION_DURATION_RATIO);
        ...
        boolean isUpwardPhase = progress < 0.5;
        correctionIsUpward = !isUpwardPhase; // 与主趋势相反
        
        // 随机生成回调参数（使用动态计算的持续时间范围）
        correctionAmplitudePercent = MIN_CORRECTION_AMPLITUDE_PERCENT + 
            random.nextDouble() * (MAX_CORRECTION_AMPLITUDE_PERCENT - MIN_CORRECTION_AMPLITUDE_PERCENT);
        ...
        LOG.debug("开始回调: 方向={}, 幅度={}%, 持续时间={}分钟 (周期{}%), 当前进度={}%",
    ...
    private double calculateConvergenceAdjustment(double progress, double baseTrendE8, double priceRangeE8) {
        ...
        if (progress < 0.5) {
            // 上涨周期：理想情况下应该在 progress=0.5 时到达 maxPrice
            idealTargetPriceE8 = minPriceE8 + progress * 2.0 * priceRangeE8;
        } else {
            // 下跌周期：理想情况下应该在 progress=1.0 时到达 minPrice
            idealTargetPriceE8 = maxPriceE8 - (progress - 0.5) * 2.0 * priceRangeE8;
        }
        ...
        // 所以需要特别关注周期中点（progress=0.5）和结束点（progress=1.0）
        boolean needsAcceleration = false;
        if (progress < 0.5) {
            // 上涨周期：在接近中点时需要确保到达 maxPrice
            // 如果进度超过 40% 但价格差距仍很大，需要加速
            if (progress > 0.4 && priceGapE8 < -priceRangeE8 * CONVERGENCE_THRESHOLD_PERCENT / 100.0) {
                needsAcceleration = true;
            }
        } else {
            // 下跌周期：在接近结束时需要确保到达 minPrice
            // 如果进度超过 90% 但价格差距仍很大，需要加速
            if (progress > 0.9 && priceGapE8 > priceRangeE8 * CONVERGENCE_THRESHOLD_PERCENT / 100.0) {
                needsAcceleration = true;
            }
        }
        
        if (needsAcceleration && gapPercent > CONVERGENCE_THRESHOLD_PERCENT) {
            // 加速因子：差距越大，加速越快
            double accelerationFactor = Math.min(1.0, (gapPercent - CONVERGENCE_THRESHOLD_PERCENT) / CONVERGENCE_THRESHOLD_PERCENT);
    ...
    private void executeTakerOrder(long targetPriceE8, short side, long currentTimeMs) {
        ...
        if (side == 0) {
            ...
            if (accountId == -1) {
                LOG.warn("Taker Buy: 买入账户组余额不足，需要 {} USDT", requiredBalanceE8 / (double) ScaleConstants.SCALE_E8);
                return;
            }
            ...
            if (accountId == -1) {
                LOG.warn("Taker Sell: 卖出账户组余额不足，需要 {} ETH", takerQtyE8 / (double) ScaleConstants.SCALE_E8);
                return;
            }
